...
// --- Insertion Rules (Adding a Key) ---

// 6. New keys are *always* inserted into a leaf node [00:05:59].

// 7. If you try to insert a key into a leaf node that is already
//    full (has 'M' keys):
//    a. The node (which temporarily has M+1 keys) must be *split* [00:05:20, 00:06:29].
//    b. The smallest half of the keys form a new left node [00:05:27].
//    c. The largest half of the keys form a new right node [00:05:34].
//    d. The *middle* key is *pushed up* into the parent node [00:05:41].

// 8. This "push-up" process is recursive [00:06:50]. If pushing a key
//    into the parent causes *that* node to become full, the parent
//    node is also split, and *its* middle key is pushed up [00:07:09].

// 9. The tree only gets taller when the *root node* splits. This
//    creates a new root node with a single key (the middle key
//    that was pushed up) [00:07:16, 00:07:31].
...
type NodeContainer<T> = [Box<T>; MAX_KEYS];

struct Node<T>
where
    T: Eq + Ord,
{
    keys: NodeContainer<T>,
    children: NodeContainer<Node<T>>,
}
